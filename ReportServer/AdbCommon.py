...
class AdbCommon():
    ...
    def getmobilebrand(self):
        ...
        try:
            ...
            logger.log_error("can not find devices." + str(e))
            return 1

    def getmobileversion(self):
        '''
        获取手机型号
        :找到返回version，1表示没找到
        '''
    ...
    def installdependapp(self):
        '''
        安装所有依赖的app
        :return: 0表示所有依赖app安装成功，1表示app安装失败
        '''
        checklist = []
    ...
    def launch_app(self, apkpackagename, activity):
        '''
        打开指定app
        packagename包名
    ...
    def click_ele(self, x_point, y_point):
        '''
        点击坐标
        '''
        if self.dev != 0:
            cmd = "adb -s %s shell input tap %s %s" % (self.dev, x_point, y_point)
            logger.log_info(cmd)
            os.system(cmd)
            time.sleep(3)
            return 0
    def input_text(self, text):
        ...
        '''
        if self.dev != 0:
            cmd = "adb -s %s shell input text %s" % (self.dev, text)
            logger.log_info(cmd)
            os.system(cmd)
            time.sleep(3)
            return 0
